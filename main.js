...
async function renderPostList(container, filterCategory = 'all') {
    ...
    if (filterCategory !== 'all') {
        postsToRender = allPosts.filter(post => {
            ...
            // Handle both English and Japanese category values
            const categoryValue = post.category[0];

            // Map filter to possible category values
            if (filterCategory === 'preparation') {
                return categoryValue === 'preparation' || categoryValue === '準備';
            } else if (filterCategory === 'thoughts') {
                return categoryValue === 'thoughts' || categoryValue === '感想';
            } else if (filterCategory === 'qanda') {
                return categoryValue === '質問集vol.1';
            }
...
async function renderArticleDetail(container) {
    ...
    // But displaying the new author info fields gracefully with a link to the member profile
    const authorLink = post.name ? `
        <span class="post-author" style="display: inline-flex; align-items: center; gap: 10px;">
            ${post.author_info || ''} ${post.name}
            <a href="members.html?name=${encodeURIComponent(post.name)}" 
               style="display: inline-block; padding: 4px 12px; background-color: #6BAF92; color: white; border-radius: 20px; font-size: 0.75rem; text-decoration: none; font-weight: bold; transition: background-color 0.3s;"
               onmouseover="this.style.backgroundColor='#5a957d'"
               onmouseout="this.style.backgroundColor='#6BAF92'">
               この人の他の記事を見る
            </a>
        </span>
    ` : `<span class="post-author">${post.author_info || '匿名'}</span>`;
    ...
    <article class="blog-post" style="max-width: 800px; margin: 0 auto;">
        <header class="post-header" style="margin-bottom: 32px; text-align: center;">
            <div style="color: #666; font-size: 0.9rem; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; flex-wrap: wrap; gap: 10px;">
                <span class="post-date">${dateDisplay}</span>
                <span style="color: #ccc;">|</span>
                ${authorLink}
                ${badgeHtml ? `<span style="color: #ccc;">|</span>${badgeHtml}` : ''}
            </div>
